...
#        help='Generating poscars and structure.pkl')
    ...
    dd_sup = DDSupercell(axis_s, H, axis, 
                         ...
                         occupation=[[0],[1],[2],[2]],
                         one_of_k_rep=False)
    dd_sup.print_settings()

    dd_enum = DDEnumeration(dd_sup, 
                            structure=st_s, 
                            active=True)
...
#    comp = [None, None, 2/3, 1/3]
    ...
    comp_lb = [None, None, None, None, None]
    comp_ub = [None, None, None, None, None]
...
#    comp_ub = [None, None, 5/8, 4/8]
    ...
    gs_all &= gs_noneq
    t2 = time.time()
    print(' number of structures (nonequiv.)   =', gs_all.len(), t2-t1)

    labelings = dd_sup.convert_graphs_to_labelings(gs_all)
    t3 = time.time()
    print(' labeling:', t3-t2)
