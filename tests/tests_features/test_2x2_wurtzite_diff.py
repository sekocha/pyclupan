...
def _init(wurtzite_primitive_cell, supercell_size: tuple, refine: bool = True):
    """Initialize ClusterFunctionsMC."""
    sup = supercell_general(
        wurtzite_primitive_cell, supercell_matrix=supercell_size, refine=refine
    )
...
def _run_test_spin_swap(cf_mc, spins):
    ...
    cf_diff12 = cf_mc.eval_from_spin_swap(spins, [0, 2])
    spins[0], spins[2] = spins[2], spins[0]
    cf_calc2 = cf_mc.eval_from_spins(spins)
    np.testing.assert_allclose(cf_calc2, cf_calc1 + cf_diff12, atol=1e-8)

    cf_diff23 = cf_mc.eval_from_spin_swap(spins, [5, 6])
    spins[5], spins[6] = spins[6], spins[5]
...
def test_eval_diff_wurtzite1(wurtzite_primitive_cell):
    """Test eval_from_spin_swap."""
    cf_mc = _init(wurtzite_primitive_cell, supercell_size=(1, 1, 2), refine=False)
    spins = np.array([1, 1, -1, 1, 1, -1, -1, -1])
    _run_test_spin_swap(cf_mc, spins)


def test_eval_diff_wurtzite2(wurtzite_primitive_cell):
    """Test eval_from_spin_swap."""
    cf_mc = _init(wurtzite_primitive_cell, supercell_size=(2, 2, 2), refine=False)
    spins = np.tile([1, 1, -1, 1, 1, -1, -1, -1], 4)
    _run_test_spin_swap(cf_mc, spins)
...
def test_eval_diff_wurtzite_spin_flip1(wurtzite_primitive_cell):
...
def test_eval_diff_wurtzite_spin_flip2(wurtzite_primitive_cell):
    """Test eval_from_spin_flip."""
    cf_mc = _init(wurtzite_primitive_cell, supercell_size=(1, 2, 4), refine=False)
    spins = np.tile([1, -1, -1, 1], 8)
    _run_test_spin_flip(cf_mc, spins)
