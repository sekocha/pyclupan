...
def test_cmc_fcc_binary():
    ...
    pyclupan.set_init(compositions=(0.8333333, 0.1666666))
    ...
    assert pyclupan.average_cluster_functions.shape == (3, 55)
...
def test_cmc_fcc_binary_from_poscar():
    ...
    size = (2, 2, 2)
    pyclupan.set_supercell(supercell_matrix=size, refine=True)
    st = Poscar(str(cwd) + "/POSCAR_init_perovskite").structure
    pyclupan.set_init(structure=st, element_strings=("Sr", "Cu", "O", "V"))
