...
class NonequivalentClusters:
    ...
    def find_nonequivalent_clusters(
        self,
        n_bodies=[1, 2],
        cutoff=[1.0],
        cutoff_cell=None,
        ...
        sup = Supercell(
            axis=niggli_axis, positions=niggli_positions, n_atoms=self.st.n_atoms, hnf=H
        )
        ...
        if n_body_ub > 1:
            for n_body in range(2, n_body_ub + 1):
                ...
                for cl_trial in sorted(cl_rep):
                    is_cutoff, _ = self.check_cluster_cutoff(
                        cl_trial, axis_s, positions_s, cut
                    )
                    if is_cutoff:
                ...
                if n_body in n_bodies:
                    ...
                    print(
                        "  ",
                        len(clusters_small),
                        str(n_body) + "-body clusters are " + "found (but eliminated).",
                    )
        ...
        if elements_lattice is not None:
            element_combs = self.find_nonequiv_element_combinations(
                clusters, perm, st_s.types, elements_lattice
            )
        ...
        for cl_idx, cl in enumerate(clusters):
            ...
            if n_body > 1:
                cut = cutoff[n_body - 2]
                _, positions_j = self.check_cluster_cutoff(cl, axis_s, positions_s, cut)
                positions_cl[:, 1:] = positions_j

            site_indices, cell_indices = [], []
            for pos in positions_cl.T:
                pos_t = np.dot(T, pos)
                cell = np.floor(pos_t + np.ones(pos_t.shape[0]) * 1e-13).astype(int)
                pos_prim = pos_t - cell
            ...
            cl_attr = Cluster(
                cl_idx + begin_cluster_id, len(site_indices), site_indices, cell_indices
            )
    ...
    def find_nonequiv_element_combinations(
        self, clusters, perm, types, elements_lattice
    ):
...
if __name__ == "__main__":
    ...
    ps.add_argument(
        "-l", "--lattice", nargs="*", type=int, default=None, help="lattice indices"
    )
    ps.add_argument(
        "-e",
        "--elements",
    ...
    ps.add_argument(
        "-n", "--n_body", type=int, default=2, help="Maximum number of sites"
    )
    args = ps.parse_args()
    ...
    if args.elements is not None:
        ...
        if args.lattice is None:
            args.lattice = [i for i, e in enumerate(args.elements) if len(e) > 1]
