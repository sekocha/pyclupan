...
def print_poscar_tofile(
    supercell, n_atoms, order, elements, remove_indices=[], filename="POSCAR"
):
    ...
    if len(remove_indices) > 0:
        ...
        for idx in remove_indices:
            ...
            positions = np.delete(positions, range(begin, end), axis=1)
...
if __name__ == "__main__":
    ...
    ps.add_argument(
        "--dump", type=str, nargs="*", default=None, help="parse derivative*.pkl file"
    )
    ps.add_argument(
        "--yaml", type=str, nargs="*", default=None, help="parse derivative*.yaml file"
    )
    ps.add_argument(
        "-r", "--random", type=int, default=None, help="number of random samples"
    )
    ps.add_argument(
        "--n_cell_ub", type=int, default=None, help="Maximum number of cells"
    )
    ps.add_argument(
        "-e", "--elements", nargs="*", type=str, default=None, help="elements"
    )
    ps.add_argument(
        "--no_poscars",
        action="store_true",
    ...
    if args.no_poscars == False:
        ...
        remove_indices = [
            i for i, e in enumerate(elements) if e == "vac" or e == "Vac" or e == "VAC"
        ]
        ...
        for i, div in enumerate(indices_split):
            ...
            for j in div:
                ...
                fname = (
                    dir2 + "poscar-" + str(n_cell) + "-" + str(s_id) + "-" + str(l_id)
                )
