...
def compute_orbits(ds_samp, n_cell, s_id, target_cluster_set, distinguish_element=True):
...
if __name__ == "__main__":
    ...
    yaml = Yaml()
    clusters, clusters_ele = yaml.parse_clusters_yaml(filename=args.clusters_yaml)
    ...
    if args.poscars is not None:
        ...
        labelings, labelings_ids = ds_samp.get_all_labelings(n_cell_ub=args.n_cell_ub)
    ...
    clusters_ele.precompute_orbit_supercell(
        cluster_set=clusters, distinguish_element=distinguish_element
    )
    ...
    t1 = time.time()
    n_all = Parallel(n_jobs=n_jobs)(
        delayed(function1)(orbit_all[ids], labelings[ids])
        for ids in sorted(labelings_ids.keys())
    )
    ...
    joblib.dump(
        (clusters_ele, target_ids, n_counts), "cluster_analysis.pkl", compress=3
    )
    if args.yaml:
