...
class Yaml:
    ...
    def write_derivative_yaml(
        self, primitive_cell, ds_set_all, filename="derivative.yaml"
    ):
    ...
    def _write_ds(self, ds_set_all, stream):
        ...
        for i, ds_set in enumerate(ds_set_all):
            ...
            for hnf, supercell, supercell_id in zip(
                ds_set.hnf_set, ds_set.supercell_set, ds_set.supercell_idset
            ):
                self._write_ds_supercell(supercell_id, hnf, stream)
                self._write_structure(supercell, stream, tag="structure", indent=4)
            ...
            print("", file=stream)
            print("  n_labelings:", ds_set.active_labelings.shape[0], file=stream)
            print("", file=stream)
    ...
    def parse_clusters_yaml(self, filename="clusters.yaml"):
        ...
        clusters = self._parse_clusters(data, tag="nonequiv_clusters", prim=self.prim)
        cluster_set = ClusterSet(clusters)
        clusters_ele = self._parse_clusters(
            data, tag="nonequiv_element_configs", prim=self.prim
        )
        ...
        cluster_set_ele = ClusterSet(clusters_ele, elements_lattice=elements_lattice)
    ...
    def _write_structure(self, st, stream, tag="primitive_cell", indent=0):
        ...
        for i in range(positions.shape[1]):
            ...
            print(
                addspace + "      coordinates:   ", list(positions[:, i]), file=stream
            )
    ...
    def write_correlations_yaml(
        ...
        for indices, c, n_atoms in zip(structure_indices, correlations, n_atoms_all):
            n_cell, s_id, l_id = indices
            print("- n_cell:        ", n_cell, file=f)
            print("  supercell_id:  ", s_id, file=f)
            print("  labeling_id:   ", l_id, file=f)
    ...
    def write_dft_yaml(self, dft_data, comp_obj=None, filename="summary_dft.yaml"):
    ...
    def write_end_members(self, comp_obj, stream):
        ...
        for i, (n_atoms, e, path) in enumerate(zip(n_atoms_end, e_end, path_end)):
            print("- id:       ", i, file=stream)
