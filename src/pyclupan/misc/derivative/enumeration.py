...
def get_nonequiv_permutation(primitive_cell, size, inactive_sites=None):
    ...
    for hnf in hnf_all:
        ...
        normalized_site_perm = np.array(sorted(set([tuple(p) for p in site_perm])))
...
if __name__ == "__main__":
    ...
    ps.add_argument(
        "--charge", type=float, nargs="*", default=None, help="Charges for elements"
    )

    ps.add_argument(
        "--hnf", type=int, nargs=9, default=None, help="Hermite normal form"
    )
    ps.add_argument(
        "-n",
        "--n_expand",
    ...
    ps.add_argument(
        "--nodump", action="store_true", help="No dump file of DSSet object"
    )
    ps.add_argument(
        "--no_labelings", action="store_true", help="Only counting number of structures"
    )
    args = ps.parse_args()
    ...
    for idx, H in enumerate(hnf_all):
        ...
        site_perm, site_perm_lt = get_permutation(st_sup, superperiodic=True, hnf=H)

        dd_const = DDConstructor(dd_handler)
        gs = dd_const.enumerate_nonequiv_configs(
        ...
        if args.no_labelings == False:
            ...
            if args.superperiodic == False:
                t2 = time.time()
                obj = pyclupancpp.NonequivLBLSuperPeriodic(
                    ds_set.all_labelings, site_perm_lt
                )
                ...
                print(
                    " number of structures (superperiodic eliminated) =",
