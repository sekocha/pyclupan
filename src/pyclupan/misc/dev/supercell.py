...
class Supercell:
    ...
    def __init__(
        ...
        self.primitive_sites = [
            i
            for i in range(self.st_prim.positions.shape[1])
    ...
    def construct_supercell(self):
        ...
        self.plattice_H = [np.dot(V, np.array(c) / np.diag(S)) for c in coord_int]

        positions_H = np.dot(Hinv, positions)
        positions_new = []
        for pos, lattice in itertools.product(*[positions_H.T, self.plattice_H]):
