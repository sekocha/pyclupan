...
class ZddLattice:
    ...
    def __init__(
        ...
        self._elements = sorted(set([e2 for e1 in elements_lattice for e2 in e1]))
        self._n_elements = len(self._elements)
        if self._verbose:
            print("Elements:", self._elements, flush=True)
        ...
        if inactive_elements is None:
            self._inactive_elements = []
        else:
            self._inactive_elements = inactive_elements
